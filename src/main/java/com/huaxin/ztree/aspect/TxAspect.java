...
public class TxAspect {
	...
	//前置通知
	@Before("execution(* com.huaxin.*.service.impl.*.*(..)) ")
	...
	//后置通知
	@After("execution(* com.huaxin.*.service.impl.*.*(..)) ")
	...
	//返回通知
	@AfterReturning(pointcut="execution(* com.huaxin.*.service.impl.*.*(..)) ",returning="res")
	...
	//异常通知[只有方法发生异常，才会执行]
	@AfterThrowing(pointcut="execution(* com.huaxin.*.service.impl.*.*(..)) ",throwing="e")
	...
	//HAUFHKANFCKAJNFKAFMKFNEWLKVNKWNVKEQVN
