...
import com.huaxin.ztree.dao.intrefaces.IUserinfoDao;
...
public class UserinfoDaoImpl extends BaseDao<Userinfo> implements IUserinfoDao {
	...
	public List<Userinfo> query(Userinfo userinfo) throws Exception {
		...
		if(userinfo.getUname()!=null && !userinfo.getUname().equals("")) {
			hql+=" and t.uname like ? ";
		}
		Query query = session.createQuery(hql);
		if(userinfo.getUname()!=null && !userinfo.getUname().equals("")) {
			query.setParameter(0, "%"+userinfo.getUname()+"%");
		}
...
/*
	@Override
	public Userinfo mohuchauxn(String userinfo) throws Exception {
		session = sessionFactory.getCurrentSession();
		String hql = " select u.uid as uid,u.uname as uname,u.sex as sex,u.telphone as telphone,"
				+ "u.utype as utype from Userinfo u where uname like ?";
		
		Query query = session.createQuery(hql);
		
		Userinfo userinfo1 = (Userinfo)query.uniqueResult();
		return userinfo1;
	}*/
